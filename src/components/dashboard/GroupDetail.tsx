  ...
  if (state.loading || !currentGroup) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20" role="status" aria-live="polite">
        ...
        <p className="text-sm text-slate-300">Loading group...</p>
      </div>
    )
  }
      ...
      <div className="flex-shrink-0">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-slate-300">{currentGroup.members.length} members · {formatCurrency(summary?.totalExpenses || 0)} total expenses</p>
          </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        <TabsContent value="overview" className="space-y-8">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-300">Total Settled</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-300">Unsettled</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              {debts.length === 0 ? (
                <p className="text-center text-slate-300 py-8">All settled up! 🎉</p>
              ) : (
                <div className="space-y-3">
                  {debts.map((debt, index) => (
                    <div key={index} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-4 sm:p-5 border border-white/15 bg-white/[0.07] rounded-xl">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-300">owes</p>
                        </div>
                        <ArrowUpDown className="h-4 w-4 text-slate-300 mx-1 shrink-0" />
                        <div className="text-right">
                          <p className="font-medium truncate">{debt.toName}</p>
                          <p className="text-sm text-slate-300">is owed</p>
                        </div>
                      ...
                      <div className="flex items-center justify-between gap-2 sm:block sm:text-right">
                        <p className="text-lg sm:text-xl font-bold text-red-400">${debt.amount.toFixed(2)}</p>
            ...
            <CardContent>
              <div className="space-y-2">
                {currentGroup.members.map(member => {
                  ...
                  return (
                    <div key={member.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/[0.1] transition-colors">
                      <div className="flex items-center space-x-3">
        ...
        <TabsContent value="expenses" className="space-y-6">
          {groupExpenses.length === 0 ? (
            <Card className="text-center py-12">
              <CardContent>
                <Receipt className="h-12 w-12 mx-auto text-white/30 mb-6" />
                <h3 className="text-lg font-medium mb-3">No expenses yet</h3>
                <p className="text-slate-300 mb-6">Add your first expense</p>
                  ...
                  <CardContent className="p-4 sm:p-5">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2"><p className="font-medium">{expense.description}</p>{expense.archivedAt && (<span className="text-[10px] px-1.5 py-0.5 rounded bg-white/25 text-white">Cleared</span>)}</div>
                          <p className="text-sm text-slate-300">{formatDate(expense.date)} · Paid by {currentGroup.members.find(m => m.id === expense.paidBy)?.name}</p>
                        </div>
                      </div>
                  ...
                  <tbody>
                    {balances.map(balance => (
                      <tr key={balance.memberId} className="border-b border-white/5">
                        ...
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-4 text-emerald-400">{formatCurrency(balance.paid)}</td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-4 text-red-400">{formatCurrency(balance.owed)}</td>
                        <td className={'text-right py-2 px-2 sm:py-3 sm:px-4 font-bold tabular-nums ' + (balance.net > 0 ? 'text-emerald-400' : balance.net < 0 ? 'text-red-400' : 'text-slate-300')}>
                  ...
                  {debts.map((debt, index) => (
                    <div key={index} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-4 sm:p-5 border border-white/15 bg-white/[0.07] rounded-xl">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium truncate">{debt.fromName}</p>
                          <p className="text-sm text-slate-300">pays</p>
                        </div>
                        <ArrowUpDown className="h-4 w-4 text-slate-300 mx-1 shrink-0" />
                        <div className="text-right">
                          <p className="font-medium truncate">{debt.toName}</p>
                          <p className="text-sm text-slate-300">receives</p>
                        </div>
                        <div className="h-9 w-9 shrink-0 rounded-full flex items-center justify-center text-sm font-medium text-white" style={{ backgroundColor: currentGroup.members.find(m => m.id === debt.to)?.color }}>
                          {debt.toName[0].toUpperCase()}
                        </div>
        ...
        <TabsContent value="settlements" className="space-y-6">
          ...
          {groupSettlements.length === 0 ? (
            <Card className="text-center py-12">
              <CardContent>
                <Handshake className="h-12 w-12 mx-auto text-white/30 mb-6" />
                <h3 className="text-lg font-medium mb-3">No settlements yet</h3>
                <p className="text-slate-300 mb-6">Record when someone pays back</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-300">{formatDateTime(settlement.date)}</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="font-bold text-lg text-emerald-400">{formatCurrency(settlement.amount)}</span>
                        {settlement.note && <span className="text-sm text-slate-300">{settlement.note}</span>}
            ...
            <CardContent>
              {(() => {
                ...
                if (allTransactions.length === 0) {
                  return <p className="text-center text-slate-300 py-8">No transactions yet</p>
                }

                return (
                  <div className="space-y-2">
                    {allTransactions.map((tx, index) => (
                      <div key={`${tx.type}-${tx.id}`} className="flex flex-wrap items-center justify-between gap-2 p-3 border-b border-white/15 last:border-0">
                        <div className="flex items-center space-x-3">
                          <div className={`h-8 w-8 rounded-full flex items-center justify-center ${tx.type === 'expense' ? 'bg-white/20' : 'bg-green-100 dark:bg-green-900/30'}`}>
                            {tx.type === 'expense' ? <DollarSign className="h-4 w-4 text-slate-300" /> : <Handshake className="h-4 w-4 text-emerald-400" />}
                          </div>
                          <div>
                            <p className="font-medium">{tx.type === 'expense' ? tx.description : `${currentGroup.members.find(m => m.id === tx.fromMemberId)?.name} → ${currentGroup.members.find(m => m.id === tx.toMemberId)?.name}`}{tx.archivedAt && <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-white/25 text-white">Cleared</span>}</p>
                            <p className="text-sm text-slate-300">{formatDateTime(tx.date)}</p>
                          </div>
                        </div>
